...
names=colnames(data)
data$pos1=ceiling(data$pos/opt$winsize)*opt$winsize
data$pos2=ceiling(data$pos/opt$winsize)*opt$winsize+opt$winsize
data$pos1[data$pos1 <=0]=0
data$range=paste(data$pos1,data$pos2,sep="-")
data<-data %>% group_by(X.chr,range) %>%
	mutate(ncount = n()) 
data<-data[,c(names,"ncount")]
print(head(data))
...
data$Gqval=p.adjust(data$Gpval, method="fdr")
data$GprimeT=max(data$Gprime[data$Gqval > opt$threshold/nrow(data)])
