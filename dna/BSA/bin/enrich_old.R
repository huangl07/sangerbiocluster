...
GO_enrich_0.05_df <- GO_enrich_0.05_df[order(GO_enrich_0.05_df$p.adjust),] #按照p.adjust排序
...
# GO_enrich_df <- GO_enrich_df[order(GO_enrich_df$logp), ]
...
p1 <- ggplot(
        GO_enrich_df,
        aes(x = GeneRatio_result, y = Description)
    ) +
        geom_point(aes(
...
# KEGG_enrich_df <- KEGG_enrich_df[order(KEGG_enrich_df$logp), ]
