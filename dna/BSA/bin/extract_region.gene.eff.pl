...
&USAGE unless ($region and $gff and $table and $fOut);
$extype||="gene";
my %region;
open IN,$region;
while(<IN>){
        ...
        foreach my $region (sort keys %{$region{$chr}}){
            ...
            if ($pos>=$pos1 && $pos<=$pos2){
                ...
                if ($type eq "SNP"){
                    ...
                    push @{$variant{$chr}{$region}},"$chr,$region\t".$_ if($impact eq "HIGH" || $impact eq "MODERATE");
                }
                if ($type eq "INDEL"){
