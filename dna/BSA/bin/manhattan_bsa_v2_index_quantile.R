...
Usage:
	...
	--mutmap  logical 是否mutmap
  --method  character num or bp
  --abs logical 是否abs
\n")
	q(status = 1)
}
...
if ("pos1" %in% colnames(pos) && opt$method == "bp") {
...
print(head(dpos))
if(opt$abs){dpos$Delta =abs(dpos$Delta)}
...
#  plot_bsa <- plot_bsa + labs(y = "Δ (SNP-index)")
  ...
  plot_bsa <- plot_bsa + labs(y = "ED4",title="manhattan plot of ED for BSA") + scale_y_continuous(limits = c(0, max(dpos$Delta)))
...
if (!is.null(opt$qcol)) {
  plot_bsa <- plot_bsa + geom_line(mapping = aes(x = BPcum, y = CI, group = order), color = "red")
#  if (opt$ylab == "delta-index") {
...
if (!is.null(opt$threshold)) {
  quantile <- quantile(dfpos$Slide, opt$threshold, na.rm = T)
...
#    plot_bsa_chr <- plot_bsa_chr + labs(y = "Δ (SNP-index)", x = i)
  ...
  if (!is.null(opt$qcol)) {
    plot_bsa_chr <- plot_bsa_chr + geom_line(mapping = aes(x = BP, y = CI), color = "red")
#    if (opt$ylab == "delta-index") {
