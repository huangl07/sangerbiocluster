...
def abstract_anno(infile: str, outfile: str, go_anno: str, kegg_anno:str) -> None:
    ...
    with open(infile, "r", encoding='utf-8') as f:
        with open(outfile, "w", encoding='utf-8') as m, \
            ...
            header = ["transcript_id", "KO_id", "ko_id", "ko_anno", "go_term", "go_anno"]
            kegg_header = ["Query", "KO", "pathway_description", "ko"]
            ...
            for line in islice(f, 1, None):
                if line.startswith("#"):
                    ...
                    transcript_info, _, _, _, _, ko_id, ko_anno, go_term, go_anno, *_, KO_id = line.strip().split("\t")
                    _, transcript_id_info, *_ = transcript_info.split(":")
                    transcript_id, *_ = transcript_id_info.split("|")
                    new_line = [transcript_id, KO_id, ko_id, ko_anno, go_term, go_anno]
                    m.write("\t".join(new_line))
                    m.write("\n")
                    new_go_term = go_term.replace(",", "; ")
                    go_new_line = [transcript_id, new_go_term]
                    n.write("\t".join(go_new_line))
                    n.write("\n")
                    ko_id_list = ko_id.split(",")
                    ko_anno_list = ko_anno.split(":")
                    if len(ko_id_list) >= 2:
                        for i in range(0, len(ko_id_list)):
                            kegg_new_line = [transcript_id, KO_id, ko_anno_list[i], ko_id_list[i]]
                            p.write("\t".join(kegg_new_line))
                            p.write("\n")
                    elif ko_id_list==[""]:
                        kegg_new_line = [transcript_id, KO_id, "-", "-"]
                        p.write("\t".join(kegg_new_line))
                        p.write("\n")
