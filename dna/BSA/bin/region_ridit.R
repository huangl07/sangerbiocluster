...
group_intervals <- function(x, threshold){
  x$ID <- rleid(x$tmp)
  x <- x %>% filter(tmp == TRUE) %>% group_by(ID) %>% filter(n() > as.numeric(opt$number))
  return(x)
}
...
result_split <- function(x){
  ...
  group_end <- dim(x)[1]
  chr <- x$chr[1]
  pos_center <- (x$pos[1] + x$pos[group_end])/2
  pos_start <- pos_center - 250000
  pos_end <- pos_center + 250000
  num_snv <- group_end
...
##输出最终结果
write.table(result, file = opt$outfile, sep = "\t", quote=FALSE, row.names=FALSE,col.names=FALSE)
