...
if(ncol(data) == 4){
	...
	df1=data[data$psites-data$pos1 < opt$stepsize & data$pos1 !=0,]
	df1$pos1=df1$pos1-opt$stepsize
	df1$pos2=df1$pos2-opt$stepsize
	data=rbind(data,df1)
	data$range=paste(data$pos1,data$pos2,sep="-")
	data<-data %>% group_by(X.chr,range) %>%
		mutate(mdepth=mean(depth),ncount=n(),sldingD=mean(delta)) %>% select(X.chr,pos,delta,depth,psites,pos1,pos2,mdepth,slidingD,ncount)
	...
	data$ptmp=data$psites-opt$winsize
	data$ptmp[data$ptmp < 0 ]=0
	data=data %>% group_by(X.chr,pos) %>% mutate(pos1=list(seq(ceiling(ptmp/opt$stepsize)*opt$stepsize,pos,by=opt$stepsize))) %>% unnest(start)
	data=data %>% group_by(X.chr,start) %>% mutate(pos2=start+opt$winsize,mdepth=mean(depth),ncount=n(),slidingI1=mean(index1),slidingI2=mean(index2),slidingED=mean(ED4),sldingD=mean(delta))
	if (is.null(opt$abs)){
