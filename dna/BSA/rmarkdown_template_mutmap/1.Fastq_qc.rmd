...
<img class="normal" src='src/err.png'>
...
```{r echo=FALSE}
library(DT)
data <- read.delim('../file/rawdata.xls',sep='\t',header=T,check.names = F)
datatable(data, rownames=F,options = list(columnDefs = list(list(className = 'dt-center',targets='_all'))))
```
...
&emsp;&emsp;测序错误率会随着测序序列长度的增加而缓慢升高，这是由于测序过程中化学试剂的消耗导致的，另外，由于 Illumina NovaSeq 6000 测序技术特点，测序片段前端几个测序循环和末端的错误率会偏高。本项目中样品的测序错误率分布图如图 3-3 所示：
...
```{r echo=FALSE}
library(DT)
data <- read.delim('../file/cleandata.xls',sep='\t',header=T,check.names = F)
datatable(data, rownames=F,options = list(columnDefs = list(list(className = 'dt-center',targets='_all'))))
```
