...
&emsp;&emsp;其中如果对EMS诱变的隐性突变基因进行分析，MutMap比较适合，野生型经EMS诱变得到突变型，突变株多代自交至纯合后与野生型杂交得到F1，F1自交得到F2，出现性状分离。
...
index_num <- data[9,2]
index_length <- data[10,2]
index_transcript <- data[11,2]
index_gene <- data[12,2]
loess_num <- data[13,2]
loess_length <- data[14,2]
loess_transcript <- data[15,2]
loess_gene <- data[16,2]
```
...
&emsp;&emsp;采用SNP-index进行关联分析:

- (1). 基于滑窗降噪的SNP-index算法共得到 `r index_num` 个与性状相关的候选区域，长度为 `r index_length` ，其中包含 `r index_transcript` 个转录本, `r index_gene` 个基因；

- (2). 基于loess拟合降噪的SNP-index算法共得到 `r loess_num` 个候选区域，长度为 `r loess_length` ，其中包含 `r loess_transcript` 个转录本, `r loess_gene` 个基因；
...
<img class="normal7" src="src/mutmap_pipeline.png">
