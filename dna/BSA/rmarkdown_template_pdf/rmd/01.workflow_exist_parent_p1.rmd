...
- (1). 野生型亲本获得 `r p1_total_base` 的 clean data，Q30 达到 `r p1_average_q30` ，测序深度为 `r p1_average_depth` ，与参考基因组平均比对效率为 `r p1_mapped_ratio` , 基因组覆盖度为 `r p1_coverage` (至少一个碱基覆盖);
...
- (1). 野生型亲本共检测到 `r prettier(p1_snp_number, 0)` 个 SNP, 其中HIGH的个数为 `r prettier(p1_snp_high_number, 0)` 个, MODERATE的个数为 `r prettier(p1_snp_moderate_number, 0)` 个; `r prettier(p1_indel_number, 0)` 个 InDel, 其中HIGH的个数为 `r prettier(p1_indel_high_number, 0)` 个, MODERATE的个数为 `r prettier(p1_indel_moderate_number, 0)` 个；
