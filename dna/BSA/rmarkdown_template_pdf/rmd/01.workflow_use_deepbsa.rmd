...
采用SNP-index、SNP-loess、欧式距离（ED）、G′ value、DeepBSA_DL、DeepBSA_K 6种算法进行关联分析:
...
- (5). 基于DeepBSA_DL算法共得到 `r deepbsa_dl_num` 个与性状相关的候选区域，长度为 `r deepbsa_dl_length` ，其中包含 `r prettier(deepbsa_dl_transcript, 0)` 个转录本, `r prettier(deepbsa_dl_gene, 0)` 个基因；

- (6). 基于DeepBSA_K算法共得到 `r deepbsa_k_num` 个与性状相关的候选区域，长度为 `r deepbsa_k_length` ，其中包含 `r prettier(deepbsa_k_transcript, 0)` 个转录本, `r prettier(deepbsa_k_gene, 0)` 个基因；
