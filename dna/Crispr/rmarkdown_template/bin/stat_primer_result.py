...
def stat_primer3_result(variant_file: str) -> pd.DataFrame:
    """
    统计primer3结果
    """
    primer_df = pd.read_csv(variant_file, sep="\t", header=0)
    primer_count = primer_df["Pos"].value_counts().shape[0]
    new_df = pd.concat([pd.Series(primer_count)], axis=1)
    new_df.rename({0:"primer_count"}, axis=1, inplace=True)
    return new_df
  
def stat_one_method_result(method: str, caps_file: str, dcaps_file: str, kasp_file: str, variant_file: str) -> pd.DataFrame:
    """
    统计一个方法的引物设计结果
    """
    ...
    primer3_count_df = stat_primer3_result(variant_file)
...
def stat_primer_design_result(indir: str, outfile: str) -> None:
    ...
    for dir in os.listdir(indir):
        if os.path.isdir(os.path.join(indir, dir)):
            ...
            variant_file = os.path.join(indir, dir, method + ".sanger.result.xls")
            one_method_result = stat_one_method_result(method, caps_file, dcaps_file, kasp_file, variant_file)
