...
spec <- matrix(c(
    ...
    "cogped", "t", 1, "character"  # 添加cogped参数
), byrow = TRUE, ncol = 4)
opt <- getopt(spec)
...
# 如果提供了cogped参数，可以进行进一步处理
if (!is.null(opt$cogped)) {
    # 处理cogped参数的逻辑
    print(paste("使用的cogped文件为：", opt$cogped))
}
...
### zdd  根据cogped抓取新生突变list
ped <- read.delim(opt$ped, header = F, sep = "\t")
# 如果提供了cogped文件，使用cogped替代ped进行处理
if (!is.null(opt$cogped) && file.info(opt$cogped)$size > 0) {
    cog_ped <- read.delim(opt$cogped, header = F, sep = "\t")
    dn_names <- cog_ped[which(cog_ped[, 6] == 2 & cog_ped[, 3] != 0 & cog_ped[, 4] != 0), 1:4]
} else {
    # 如果没有提供cogped，使用原始ped文件
...
if (length(genes_eg) > 0) {
    ...
    if (!is.null(enrich_df)) {
        ...
        #########zdd
        enrich_df$gene_name <- sapply(enrich_df$geneID, function(entrez_ids) {
        entrez_ids_list <- unlist(strsplit(entrez_ids, "/"))
        gene_names <- names(entrenz_list)[entrenz_list %in% entrez_ids_list]
        if (length(gene_names) > 0) {
            return(paste(gene_names, collapse = "/"))
        } else {
            return(NA)
        }})
        colnames(enrich_df)[which(colnames(enrich_df) == "geneID")] <- "EntrezID"
        #########zdd
...
if (length(genes_eg) > 0) {
    ...
    if (!is.null(go_enrich)) {
        ...
        #########zdd
        go_enrich_df$gene_name <- sapply(go_enrich_df$geneID, function(entrez_ids) {
        entrez_ids_list <- unlist(strsplit(entrez_ids, "/"))
        gene_names <- names(entrenz_list)[entrenz_list %in% entrez_ids_list]
        if (length(gene_names) > 0) {
            return(paste(gene_names, collapse = "/"))
        ...
        colnames(go_enrich_df)[which(colnames(go_enrich_df) == "geneID")] <- "EntrezID"
        #########zdd
...
if (length(genes_eg) > 0) {
    ...
    if (!is.null(kegg_enrich)) {
        ...
        #########zdd
        kegg_enrich_df$gene_name <- sapply(kegg_enrich_df$geneID, function(entrez_ids) {
        entrez_ids_list <- unlist(strsplit(entrez_ids, "/"))
        gene_names <- names(entrenz_list)[entrenz_list %in% entrez_ids_list]
        if (length(gene_names) > 0) {
            return(paste(gene_names, collapse = "/"))
        ...
        colnames(kegg_enrich_df)[which(colnames(kegg_enrich_df) == "geneID")] <- "EntrezID"
        #########zdd
