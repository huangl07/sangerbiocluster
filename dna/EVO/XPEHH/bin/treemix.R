...
Options: 
	...
	--inpout 	character 	the input file [forced]
	--output 	character 	the filename for output graph [forced]
...
tbl=OptM:::read.treemix(opt$input,F)
if(nlevels(as.factor(tbl$M)) < 3){ ## for somtime treemix only return 1 or 0
    best=1
}else{
    test.optM = optM(opt$input)
    colnames(test.optM)=c("m","runs","mean","sd","min","max","Lm","sdL","minLm","maxLm","LLm","sdllm","minllm","maxllm","Deltam","meanf","sdf")
    p1=ggplot(test.optM)+geom_point(aes(x=m,y=Deltam),col="blue")+geom_line(aes(x=m,y=Deltam),col="blue")+theme_bw()
    p2=ggplot(test.optM)+geom_boxplot(aes(x=m,middle=mean,lower=min,upper=max,ymin=mean+sd,ymax=mean+sd,group=m),stat="identity")
    p=ggarrange(p1,p1)
    ggsave(p,file=paste(opt$output,"OptM.pdf",sep="."),device="pdf")
    ggsave(p,file=paste(opt$output,"OptM.png",sep="."),device="png")
    write.table(file=paste(opt$output,"OptM.csv",sep="."),test.optM,sep=",",quote=F,row.names=F)
    best=test.optM$m[which.max(test.optM$Deltam)]
}
p=plot.treemix(paste(opt$input,"/pop.",best,".1",sep=""))
dp=p$plot+theme_classic()+theme(axis.line.y=element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank(),title=element_blank())
ggsave(dp,file=paste(opt$output,best,"best.tree.pdf",sep="."),device="pdf")
ggsave(dp,file=paste(opt$output,best,"best.tree.png",sep="."),device="png")
library(reshape2)
matrix<-read.table(paste(opt$input,"/pop",best,"1.cov.gz",sep="."))
matrix$id=rownames(matrix)
df=melt(matrix)
p=ggplot(df)+geom_tile(aes(x=id,y=variable,fill=value))
p=p+geom_text(aes(x=id,y=variable,label=ceiling(value*10000)/10000))+scale_fill_gradient2(low = "darkblue",high="yellow",mid="white")
ggsave(p,file=paste(opt$output,best,"best.matrix.pdf",sep="."),device="pdf")
ggsave(p,file=paste(opt$output,best,"best.matrix.png",sep="."),device="png")
