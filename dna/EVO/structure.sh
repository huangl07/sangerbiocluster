...
for i in {1..10}
do
    echo -e "less K${$i}.popfile|cut -f ${i} > K${$i}.newpop"
done|xargs -i -P 8 bash -c '{}'

for i in {1..10}
do
    echo -e "distructLinux1.1 -K ${i} -d   ~/sg-users/long.huang/STRUCTURE/drawparams -i K${i}.outfile -p K${i}.newpop -N ${N} -M ${i} -o K${i}"
done