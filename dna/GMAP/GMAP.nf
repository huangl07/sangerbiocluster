...
def helpMessage() {
    ...
    --vcf   <file>  vcf相对wgs_result的相对路径，自定义vcf时才修改该参数 published/data/04.snpIndel/finalvcf/pop.snpindel.final.vcf.gz
    --wgs   <dir>   wgs output dir output
...
vcf=file(params.vcf)
if(params.wgs){
    vcf=file("${params.wgs}/published/data/04.snpIndel/finalvcf/pop.snpindel.final.vcf.gz")
}
chr=file(params.chr)
if(params.SSR){
    SSR=file(params.SSR)
...
if(params.SSR){
    log.info("SSR parameters means chr_only not effective")
    params.chr_only=0
}
if(params.SSR){
    process vcf2table{
        ...
        input:
            ...
            file SSR from SSR
        output:
            file "pop.final.marker" into filtered
            file "pop.filtered.stat" into filterstat
            file "*"
            file "variant.diff.table" into variant_diff_table_ch
            file "genome.size" into genome_size
        script:
            """
            source ~/app/bioinfo/dna/new.rc
            echo ${params.p1} > sample.txt
            echo ${params.p2} >> sample.txt
            tabix ${vcf}
            bcftools +setGT --threads 8 ${vcf} -- -t q -n . -i 'FMT/DP<${params.Odep}'| bcftools filter --threads 8 -i "MIN(FORMAT/DP[@sample.txt]) > ${params.Pdep} " |bcftools query -H -f "%CHROM-%POS\\t[%GT\\t]\\n" > pop.table
            bcftools view -h ${vcf} | grep \"^##contig\" | sed -e \'s/.*ID=\\([^,>]*\\).*/\\1/g' > id.list
            bcftools view -h ${vcf} | grep \"^##contig\" | sed -e 's/.*length=\\([0-9]*\\).*/\\1/g' > length.list
            paste id.list length.list > genome.size
            perl ${baseDir}/bin/table2marker.pl -input pop.table -output pop.primary.marker -PID ${params.p1} -MID ${params.p2}
            ...
            less -S pop.filtered.marker|sed 's/-/\\t/g'|cut -f 1,3|sort|uniq -c > marker.filter.type.stat
            perl ${baseDir}/bin/mergeSSR.pl -input pop.filtered.marker -SSR ${params.SSR} -out pop.filtered.merge.marker
            ln -s pop.filtered.merge.marker pop.final.marker
            perl ${baseDir}/bin/filter_stat.pl -input pop.filtered.merge.marker -output pop.filtered.stat --vcf ${vcf}
            bcftools view -s ${params.p1},${params.p2} ${vcf} | bcftools query -H -f "%CHROM\\t%POS\\t%REF\\t%ALT[\\t%TGT][\\t%AD]\\t%ANN\\n" | perl -ne 'next if(/#/);chomp;@a=split;@b=split(/\\,/,\$a[-1]);foreach \$b(@b){@c=split(/\\|/,\$b);print join("\\t",@a[0..\$#a-1],@c),"\\n";}' > variant.table
    ...
    script:
        ...
        perl ${baseDir}/bin/markerfilter.pl -input pop.primary.marker -output pop  -mis ${params.missing} -seg ${params.segment} -popt ${params.popt}
        less -S pop.filtered.marker|sed 's/-/\\t/g'|cut -f 1,3|sort|uniq -c > marker.filter.type.stat
        ln -s pop.filtered.marker pop.final.marker
        perl ${baseDir}/bin/filter_stat.pl -input pop.filtered.marker -output pop.filtered.stat --vcf ${vcf}
        bcftools view -s ${params.p1},${params.p2} ${vcf} | bcftools query -H -f "%CHROM\\t%POS\\t%REF\\t%ALT[\\t%TGT][\\t%AD]\\t%ANN\\n" | perl -ne 'next if(/#/);chomp;@a=split;@b=split(/\\,/,\$a[-1]);foreach \$b(@b){@c=split(/\\|/,\$b);print join("\\t",@a[0..\$#a-1],@c),"\\n";}' > variant.table
        awk '{if((\$6 != "./." || \$5 != "./.") && \$5 != \$6) {safe++;print \$0}}' variant.table > variant.diff.table
        sed -i "1i CHROM\\tPOS\\tREF\\tALT\\t${params.p1}:GT\\t${params.p2}:GT\\t${params.p1}:AD\\t${params.p2}:AD\\tANN_Allele\\tANN_Annotation\\tAnnotation_impact\\tGene_Name\\tGene_ID\\tFeature_Type\\tFeature_ID\\tTranscript_BioType\\tRank\\tHGVS.c\\tHVGS.p\\tcDNA_stat\\tCDS_stat\\tAA_stat\\tDistance\\tERRORS" variant.diff.table
        """
    ...
    if(params.popt == "CP"){
      if(params.chr){
        ...
        if(params.chr){
            """
            source ~/app/bioinfo/dna/new.rc
            perl ${baseDir}/bin/markersplitNOCP.pl -input ${marker} -fOut scaf.list -dOut split -chr ${chrfile} --size ${size}
            """
        }else{
            """
            source ~/app/bioinfo/dna/new.rc
            perl ${baseDir}/bin/markersplitNOCP.pl -input ${marker} -fOut scaf.list -dOut split --size ${size}
            """
        }
    }
}
...
process mapping{
    ...
    if(params.popt == "CP"){
        if(params.chr_only){
            """
            source ~/app/bioinfo/dna/new.rc
            crosslink_group --inp=${marker[0]} --outbase=${lg}. --knn=30 --matpat_lod=20 --matpat_weights=01P07 --min_lod=0 --redundancy_lod=20 --redun=${lg}.redun.list
            ...
            cut -f 1 -d " " ${lg}.draw.loc|perl -ne 'chomp;@a=split(/-/,\$_);print \$_,"\\t",\$a[1],"\\n";' > ref.map
			perl ${baseDir}/bin/smooth-CP.pl -l ${lg}.draw.loc -k ${lg} -d ./ -m ref.map   -ami 0.8
            crosslink_group --inp=${lg}.correct.loc --outbase=${lg}.correct. --knn=45 --map=${lg}.correct. --min_lod=0
            ln -s  `wc -l ${lg}.correct.*.loc|sort -nr|head -n2|tail -n 1|awk '{print \$2}'` ${lg}.result.csv
            ln -s  `wc -l ${lg}.correct.*.map|sort -nr|head -n2|tail -n 1|awk '{print \$2}'` ${lg}.tmp.map
            less -S ${lg}.tmp.map|grep -v group|perl -ne 'if(/group/){print "group\\t${lg}\\n"}else{@a=split;print join("\\t",@a),"\\n"}' > ${lg}.tmp.format.map
            Rscript ${baseDir}/bin/genericMap.R --input ${lg}.tmp.format.map --output ${lg}.result.map
            ...
            """
            source ~/app/bioinfo/dna/new.rc
            crosslink_group --inp=${marker[0]} --outbase=${lg}. --knn=30 --matpat_lod=20 --matpat_weights=01P07 --min_lod=0 --redundancy_lod=20
