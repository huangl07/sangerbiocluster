...
while (<IN>) {
	chomp;
	next if (/Type/ || /type/ || /^;/ || /^$/ || /individual/ || /Individual/ || /^#/ || /MarkerID/ || /chr-pos/);##<  
	my @in=split /\s+/,$_;
	if ($in[1]=~/lmxll/ | /nnxnp/){  
		$all1{$k}{"genotype_num"}=1;
		$all1{$k}{"marker"}=$_;
	}elsif($in[1]=~/efxeg/ | /abxcd/){
		$all1{$k}{"genotype_num"}=3;
		$all1{$k}{"marker"}=$_;
	}else{
		$all1{$k}{"genotype_num"}=2;
		$all1{$k}{"marker"}=$_;
	}
    $k++;
}
...
while (<IN>) {
	...
	my @in=split /\s+/,$_;
	if ($in[1]=~/lmxll/ | /nnxnp/){  
		$all2{$k}{"genotype_num"}=1;
		$all2{$k}{"marker"}=$_;
	}elsif($in[1]=~/efxeg/ | /abxcd/){
		$all2{$k}{"genotype_num"}=3;
		$all2{$k}{"marker"}=$_;
	}else{
		$all2{$k}{"genotype_num"}=2;
		$all2{$k}{"marker"}=$_;
	}
	$k++;
}
...
#------------------------------------------------------------------
my $all1=scalar keys %all1;
my $all2=scalar keys %all2;
...
for (my $i=0; $i<$all1; $i++) {
	for (my $j=0; $j<$all2; $j++) {
		my (%pairs,%R,%C);
        my ($marker1,$genotype1,@indi1,$marker2,$genotype2,@indi2);
		    ($marker1,$genotype1,@indi1)=split /\s+/,$all1{$i}{"marker"};
		    ($marker2,$genotype2,@indi2)=split /\s+/,$all2{$j}{"marker"};
...
Usage:
  Options:
  -i1   <file> input file,genotype file,forced 
  -i2   <file>  input file genotype2 file ,forced
  -o    <fiel>	output file,hold result of mlod
  -popt <str>   str poptype for calculate
  -h         Help
