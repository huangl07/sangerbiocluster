...
d<-read.table(opt$mark,head=F,sep=",")
d<-na.omit(d)
...
plot(5, type = "n", xlim = c(0, d$end[which.max(d$end)]), ylim = c(0,ncol(d) - 4), xaxt = "n",main="Haplotype")
...
for (i in chr){
	...
	pos=c(1:length(subd$pos[subd$chr==i])/2)
	...
	for (j in 4:ncol(subd)) {
		...
		rect(xleft = subd$start, ybottom = j - 5, xright = subd$end,ytop = j - 4.6, col = cols[as.character(subd[seq(0,length(subd[,j]),2),j])], border = NA)
		rect(xleft = subd$start, ybottom = j - 4.6, xright = subd$end,ytop = j - 4.2, col = cols[as.character(subd[seq(1,length(subd[,j]),2),j])], border = NA)
