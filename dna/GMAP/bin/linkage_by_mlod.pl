...
my @sub_solution = ();
my $tree_sum = select_sub_tree($Tree,$minGroup,$maxGroup,$start,$end,\@sub_solution);
...
sub construct_group_tree {#
	...
	$tree->{'root'} = init_node([keys %{$loci}],0);
	push @queue,$tree->{'root'};
...
sub select_sub_tree {#
	my ($tree,$minG,$maxG,$b_lod,$e_lod,$solution) = @_;
