...
manhattan(d,chr="GroupID",bp="Dis",p="pvalue",snp="X.MarkerID",ylim=c(0,ymax),logp=FALSE,col=rainbow(4),suggestiveline=log(0.05)/log(10)*-1,genomewideline=FALSE,ylab="Segregation Pvalue",main="Marker Disortion Distribution")
box()
dev.off()
...
for(i in chr){
	df=d %>% filter(GroupID == i)
	pdf(paste(opt$out,".LG",i,".seg.pdf",sep=""),height=900,width=1600)
	manhattan(df,chr="GroupID",bp="Dis",p="pvalue",snp="X.MarkerID",ylim=c(0,ymax),logp=FALSE,col=rainbow(4),suggestiveline=log(0.05)/log(10)*-1,genomewideline=FALSE,ylab="Segregation Pvalue",main="Marker Disortion Distribution")
	box()
	dev.off()
