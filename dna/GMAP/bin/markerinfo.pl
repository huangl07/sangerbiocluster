...
close Region;
if(scalar keys %binstat != 0 ){
	open Out,">$fOut.bin.stat";
	print Out "#CHROM\tBinNum\tAverBinSize\n";
	foreach my $chr(sort keys %binstat){
		print Out $chr,"\t",$binstat{$chr}{num},"\t",$binstat{$chr}{len}/$binstat{$chr}{num},"\n";
	}
	close Out;
}
open Out,">$fOut\.type";
