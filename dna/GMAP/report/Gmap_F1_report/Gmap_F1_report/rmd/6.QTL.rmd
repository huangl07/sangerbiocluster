...
&emsp;&emsp;结合LP_num个连锁群的图谱和标记分型数据， 以及Trait_num个性状表型值，采用软件 R/qtl$^{[2]}$的进行QTL分析。具体分析方法如下：

&emsp;&emsp;(1) 利用区间作图法（ Interval Mapping )$^{[7]}$进行 QTL 定位；

&emsp;&emsp;(2) 利用PT 检验（Permutation test,1000次）确定阈值，当置信水平大于95%时的LOD值为QTL筛选阈值，若该阈值下没有获得定位区域，可适当降低。

&emsp;&emsp; 各性状定位结果如表3-17所示。其中性状的定位结果如图3-11所示，其他表型定位结果见附件。

<p class='mark'>表3-17 性状QTL定位区间及基因筛选结果</p>
...
<p class="mark">图3-11 性状QTL定位结果</p>
...
&emsp;&emsp;根据标记在染色体上的位置信息，确定关联区域的标记和功能，其中有蛋白结构编码影响的分子标记是我们建议重点关注的标记。根据性状的定位结果，寻找性状候选区域内的基因，各个性状的定位区域及基因统计见表3-18和3-19。


<p class='mark'>表3-18 各性状定位区间信息统计</p>
...
<p class='mark'>表3-19 各性状定位区间信息统计</p>

```{r echo=FALSE}///html
library(DT)///html
data <- read.delim('file/t.region.xls',sep='\t',header=T,check.names = F)///html
datatable(data, rownames=F,options = list(scrollX=T,columnDefs = list(list(className = 'dt-center',targets='_all'))))///html
```///html
...
&emsp;&emsp;在候选区域内，共注释到数据库的基因具体统计结果见表3-20。


<p class='mark'>表3-20 基因功能注释结果列表</p>
...
```{r echo=FALSE}///html
library(slickR)///html
state_plots <- list.files("file","genes_ORA_GOenrichment_bubble.png",full.names=TRUE)///html
slickR(obj=state_plots,height=450,width='95%',)///html
```///html
...
```{r echo=FALSE}///html
library(slickR)///html
state_plots <- list.files("file","genes_ORA_KEGGenrichment_bubble.png",full.names=TRUE)///html
slickR(obj=state_plots,height=450,width='95%',)///html
```///html
...
<p class="mark">图3-13 候选区域内基因的通路分布图</p>

**注：纵坐标为KEGG的代谢通路，横坐标为富集因子，图中点的大小代表基因数目，点颜色代表-log10(padjust)。**

&emsp;&emsp;候选区域KEGG的富集分析结果见下表：

<p class='mark'>表3-22 候选区域内基因的KEGG富集部分结果</p>

```{r echo=FALSE}///html
library(DT)///html
data <- read.delim('file/genes_ORA_KEGGenrichment.xls',sep='\t',header=T,check.names = F)///html
datatable(data, rownames=F,options = list(scrollX=T,columnDefs = list(list(className = 'dt-center',targets='_all'))))///html
```///html
