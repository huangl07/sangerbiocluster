...
if '5' not in exclude:
    Step1Sh.write('cp %s/result/01.genotype/pop.type.type.stat.xls  %s/file/ \n\n' %(result_dir,report_dir))
    Step1Sh.write('cp %s/result/01.genotype/pop.type.type.stat.xls  %s/file/ \n\n' %(result_dir,report_dir))
    Step1Sh.write('cp %s/result/03.gmap_evaluate/total.map.png  %s/file/ \n\n' %(result_dir,report_dir))
    Step1Sh.write('''less %s/result/02.gmap/total.mapstat.xls|grep -v -E "：|:"|sed 's/#//g'  >%s/file/total.mapstat.xls \n\n'''%(result_dir,report_dir))
    Step1Sh.write('cp %s/result/03.gmap_evaluate/total.bin.png  %s/file/ \n\n' %(result_dir,report_dir))    
    Step1Sh.write('cp %s/result/03.gmap_evaluate/1.heatMap.png  %s/file/ \n\n' %(result_dir,report_dir)) 
    Step1Sh.write('cp %s/result/03.gmap_evaluate/total.phy.png  %s/file/ \n\n' %(result_dir,report_dir)) 
    Step1Sh.write('''less %s/result/03.gmap_evaluate/total.phy.spearman.xls|sed 's/^#//g'  >  %s/file/total.phy.spearman.xls \n\n''' %(result_dir,report_dir)) 

if '6' not in exclude:
#    order=18
    filename_t=os.path.split(glob.glob(os.path.join(result_dir,'result/04.qtl/*qtl.csv'))[0])[1]
    filename_p=os.path.split(glob.glob(os.path.join(result_dir,'result/04.qtl/*scan.png'))[0])[1]
    filename_anno=os.path.split(glob.glob(os.path.join(result_dir,'result/05.anno/*gene.total'))[0])[1]
    #filename_pos=os.path.split(glob.glob(os.path.join(result_dir,'result/05.anno/*gene.eff'))[0])[1]
    filename_vcf=os.path.split(glob.glob(os.path.join(result_dir,'result/05.anno/*vcf.total'))[0])[1]
    Step1Sh.write('''less %s/result/04.qtl/%s|sed 's/,/\t/g' > %s/file/t.qtl.xls \n\n''' %(result_dir,filename_t,report_dir))
    Step1Sh.write('cp %s/result/04.qtl/%s  %s/file/t.scan.png \n\n' %(result_dir,filename_p,report_dir))
    Step1Sh.write('''less %s/result/05.anno/%s|grep -E '^@|^#@'|sed -e 's/@//g' -e 's/#//g'|grep -E 'pos|chr' > %s/file/t.gene_anno.xls \n\n''' %(result_dir,filename_anno,report_dir))
    #Step1Sh.write('''less %s/result/05.anno/%s|grep -E '^@|^#@'|sed -e 's/@//g' -e 's/#//g' > %s/file/t.region.xls \n\n''' %(result_dir,filename_pos,report_dir))
    Step1Sh.write('''less %s/result/05.anno/%s|grep -E '^@|^#@'|sed  -e 's/@//g' -e 's/#//g'|grep -E 'pos|chr' > %s/file/region.anno.xls \n\n''' %(result_dir,filename_vcf,report_dir))
    Step1Sh.write('cp %s/result/06.enrich/GO_result/genes_GOenrichment_bubble.png  %s/file/ \n\n' %(result_dir,report_dir))
    Step1Sh.write('cp %s/result/06.enrich/GO_result/genes_GOenrichment.xls  %s/file/ \n\n' %(result_dir,report_dir))
    Step1Sh.write('cp %s/result/06.enrich/KEGG_result/genes_KEGGenrichment_bubble.png  %s/file/ \n\n' %(result_dir,report_dir))
    Step1Sh.write('cp %s/result/06.enrich/KEGG_result/genes_KEGGenrichment.xls  %s/file/ \n\n' %(result_dir,report_dir))
