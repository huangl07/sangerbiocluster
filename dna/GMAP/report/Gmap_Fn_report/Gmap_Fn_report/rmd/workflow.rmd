...
&emsp;&emsp;本项目利用全基因组重测序(Whole Genome Sequencing, WGS)技术对Pop_type群体（2 个亲本和 多个子代）开发全基因组范围内的 SNP 和 InDel 标记，通过ASMap构建遗传图谱，利用R/qtl 软件$^{[1]}$根据遗传图谱进行QTL 定位并筛选候选基因。
...
- 1、全基因组重测序，每个样本平均数据量 Bulk_rawdata , Q30≥ 80%；
...
&emsp;&emsp;本次测序共获得 Cleandata_size 数据量 ，Q30 达到 Q30_num ，平均测序深度为 Bulk_deoth 。样品与参考基因组平均比对效率为 Mapping_ratio， 平均覆盖深度为 Avg_depth ，基因组覆盖度为 Ref_coverage  （至少一个碱基覆盖）。
...
&emsp;&emsp;通过对SNP及Indel标记的筛选与过滤：共剩余total_fileter_num个标记用于后续图谱构建，并通过使用bin的方法进行构建。最终上图（bin）的标记数为Bin_num个，分布于LP_num个连锁群上，单条连锁群最大gap GAP_cM cM。对Trait_num个数量性状表型值进行QTL 定位分析，获得Association_num个关联区域。
