...
```{r load_common_data}
wgs_result_dir <- normalizePath(params$wgs_result)
gmap_result_dir <- normalizePath(params$gmap_result)
data_release_dir <- normalizePath(params$data_release)
project_info_file <- file.path(wgs_result_dir,"output","tmp","02.reference", "project.info") #读取project.info信息文件
genome_info_file <- file.path(wgs_result_dir,"output","tmp", "02.reference", "info.log") # 读取info.log信息文件
...
cat(
  knitr::knit_child(paste0(rmd,'1.workflow.rmd'), envir = globalenv(), quiet = TRUE)
  )
```
```{r fastq-qc-module, results='asis'}
cat(
  knitr::knit_child(paste0(rmd,'2.Fastq_qc.rmd'), envir = globalenv(), quiet = TRUE)
  )
```
```{r mapping-module, results='asis'}
cat(
  knitr::knit_child(paste0(rmd,'3.Mapping.rmd'), envir = globalenv(), quiet = TRUE)
  )
```
```{r snp-module, results='asis'}
cat(
  knitr::knit_child(paste0(rmd,'4.SNV.rmd'), envir = globalenv(), quiet = TRUE)
  )
```
```{r genetic-map-module, results='asis'}
cat(
  knitr::knit_child(paste0(rmd,'5.GeneticMap.rmd'), envir = globalenv(), quiet = TRUE)
  )
```
```{r qtl-module, results='asis',eval=params$run_qtl=="yes"}
cat(
    knitr::knit_child(paste0(rmd,'7.QTL.rmd'), envir = globalenv(), quiet = TRUE)
  )
```
```{r appendix-module, results='asis'}
cat(
  knitr::knit_child(paste0(rmd,'99.appendix.rmd'), envir = globalenv(), quiet = TRUE)
  )
```
```{r, include=pdffmt}
knitr::asis_output('\\newpage\n\n\\newpage\n')
