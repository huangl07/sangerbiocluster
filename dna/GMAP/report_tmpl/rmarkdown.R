...
argv <- add_argument(argv,"--wgs_result", help="有参变异检测结果路径")
argv <- add_argument(argv,"--gmap_result", help="遗传图谱分析路径")
argv <- add_argument(argv,"--data_release", help="遗传图谱释放文件夹")
argv <- add_argument(argv,"--outfile", help="the output file")
argv <- add_argument(argv,"--run_qtl", help="是否允许qtl")
argv <- add_argument(argv,"--type", help="wgs/rad/gbs")
...
if (format=='html') {
    render(input=rmdfile,
        ...
        params = list(
            docfmt = "html",
            devmode = argv$dev,
            wgs_result = argv$wgs_result,
            gmap_result = argv$gmap_result,
            data_release = argv$data_release,
            run_qtl = argv$run_qtl,
            type = argv$type
    )
)} else if (format=='pdf') {
    render(input=rmdfile,
