...
knitr::asis_output(sprintf(paste(readLines("tex/first_page_gbs.tex"),collapse="\n"), contract_number, genome_name, sample_number))
```
...
RAD-seq (Restriction-site Associated DNA Sequencing, 限制酶切位点关联DNA测序) 是一种基于第二代测序技术的发展而来的简化基因组测序方法。本项目利用简化基因组技术，开发全基因组范围内的SNP分子标记，利用SNP标记进行群体遗传学研究。

## 材料基本信息

```{r get-info-table}
qc_stat_file <- file.path(wgs_result_dir,"output","tmp", "01.fastq_qc", "qc.stat")
snp_stat_all_file <- file.path(wgs_result_dir,"output","published","data","04.snpIndel", "snp", "snp.stat.all.xls")
qc_stat <- read.delim(qc_stat_file,sep='\t',header=FALSE,check.names = FALSE,comment="#")
snp_stat_all <- read.delim(snp_stat_all_file,sep='\t',header=TRUE,check.names = FALSE)
```
...
1. 简化基因组重测序，每个样本测序量达到合同标准，Q30 $\geq$ 80%。

2. 比对参考基因组进行变异检测分析，具体内容包括：SNP 检测和注释、InDel 检测和注释；

3. 遗传图谱构建：分子标记筛选过滤，binmarker构建，遗传图谱构建，图谱质量评估。

## 分析结果概述

本项目共获得`r prettier(sum(qc_stat[,3] / 1000000000),2)`G reads数据，测序Q30为`r prettier(mean(qc_stat[,5] * 100), 2)`%，
GC含量为`r prettier(mean(qc_stat[,6] * 100), 2)`%。通过生物信息学分析，共获得`r prettier(snp_stat_all$total[1])`个SNP。
...
```{r library-plot,fig.cap = "简化基因组重测序实验建库流程"}
include_graphics(ifelse(platform != "DNBSeq", "static/images/library_GBS.png", "static/images/dnbseq.png"))
```
...
include_graphics("static/images/pipeline.png")
```
