...
dt_caption("简化基因组区域SNP位置分布信息统计表","snpannostat-table")
...
tempdf[,1]=gsub("_","\\\\_",tempdf[,1])
kbl(row.names=FALSE, tempdf, "latex", caption="简化基因组区域SNP位置分布信息统计表", booktabs = TRUE, longtable = TRUE, linesep=' ',col.names=linebreak(gsub(" ","\n",colnames(tempdf)),align="c"),escape=FALSE) %>%
kable_styling(repeat_header_text="(续)", latex_options=c("striped","scale_down","repeat_header"))%>%
kbl_foot(tempfn) %>% table_bline()
```
...
dt_caption("简化基因组区域SNP功效信息统计表","effectstat-table")
datatable(tempdf, rownames=FALSE,options = list(scrollX=T,columnDefs = list(list(className = 'dt-center',targets='_all'))))
dt_foot(tempfn)
```

```{r effectstat-table,eval=pdffmt}
kbl(row.names=FALSE, tempdf, "latex", caption="简化基因组区域SNP功效信息统计表", booktabs = TRUE, longtable = TRUE, linesep=' ') %>%
    kable_styling(repeat_header_text="(续)", latex_options=c("striped","scale_down","repeat_header"))%>%
    kbl_foot(tempfn) %>% table_bline()
```
...
dt_caption("简化基因组区域InDel位置分布信息统计表","indelannostat-table")
...
kbl(row.names=FALSE, arrange_indel_anno, "latex", caption="简化基因组区域InDel位置分布信息统计表", booktabs = TRUE, longtable = TRUE, linesep=' ',col.names=linebreak(gsub(" ","\n",colnames(arrange_indel_anno)),align="c"),escape=FALSE) %>%
  kable_styling(repeat_header_text="(续)", latex_options=c("striped","scale_down","repeat_header"))%>%
  kbl_foot(tempfn) %>%
  table_bline()
```
...
dt_caption("简化基因组区域InDel功效信息统计表","indeleffectstat-table")
...
kbl(row.names=FALSE, arrange_indel_impact, "latex", caption="简化基因组区域InDel功效信息统计表", booktabs = TRUE, longtable = TRUE, linesep=' ') %>%
  kable_styling(repeat_header_text="(续)", latex_options=c("striped","scale_down","repeat_header"))%>%
  kbl_foot(tempfn) %>%
  table_bline()
```
