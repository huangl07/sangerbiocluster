...
def align2genome(R1,R2,genome,outfile,thread,directory):
    ...
    cmd1 = "bwa-mem2 index "+ genome
    cmd2 = "bwa-mem2 mem -T 20 -t "+ str(thread) +" "+ genome + " " + R1 + " "+ R2 + " >"+outfile+".sam"
...
def fileSplit(tdnaSAM,tmp_dir):
    ...
    cmd2 = "python3 "+ os.path.join(bindir, "FileSplitter.py") + " "+ tmp_sam +" 10000"
    #print(cmd2)
...
def detectZygosity(insertionbag,fq1,fq2,reference,thread,directory,project):
    ...
    with open(insertionBED,"w") as file_bed:
        ...
        if len(insertionbag) == 0:
            print("No insertion is found")
            return
        seq_region = ""
        ...
        cmd1 = 'samtools faidx -c '+reference+' '+seq_region+' >'+insertionSeq
        #print(cmd1)
        run(cmd1)
        cmd2 = "bwa-mem2 index "+insertionSeq
        cmd3 = "bwa-mem2 mem -T 20 -t "+ str(thread) +" "+ insertionSeq + " " + fq1 + " "+ fq2 + " >"+outfile+".sam"
