...
###导入所有的gene_list
all_gene_list <- read.delim(opt$genefile, header = TRUE, col.names = c("chr", "gene_start", "gene_end", "gene_id"), colClasses=c("character", "numeric", "numeric", "character")) %>% 
  as_tibble()
...
abstract_df <- function(x, pos_start, pos_end, all_gene_list){
  df_abstract <- all_gene_list %>%
    ...
    select(REGION, chr, gene_start, gene_end, gene_id)
  return(df_abstract)
}
