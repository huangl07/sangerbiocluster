...
if (nrow(GO_enrich_df) == 0){write("No GO enrichment Result",file="./GO.error.log")}else{
if (nrow(GO_enrich_df)>1){GO_enrich_df <- GO_enrich_df[order(GO_enrich_df$p.adjust),] }#按照p.adjust排序
GO_enrich_0.05_df <- as.data.frame(GO_enrich_0.05)
if (nrow(GO_enrich_0.05_df)>1){GO_enrich_0.05_df <- GO_enrich_0.05_df[order(GO_enrich_0.05_df$p.adjust),]} #按照p.adjust排序
...
if (nrow(GO_enrich_df)>1){GO_enrich_df <- GO_enrich_df[order(GO_enrich_df$ONTOLOGY),]}
...
if(sum(DEG %in% term2gene$V2)==0){
  ...
  if (nrow(KEGG_enrich_df) == 0){write("No KEGG enrichment Result",file="./KEGG.error.log")}else{
  if (nrow(KEGG_enrich_df)>1){KEGG_enrich_df <- KEGG_enrich_df[order(KEGG_enrich_df$p.adjust),]}#按照p.adjust排序
  KEGG_enrich_0.05_df <- as.data.frame(KEGG_enrich_0.05)
  if (nrow(KEGG_enrich_0.05_df)>1){KEGG_enrich_0.05_df <- KEGG_enrich_0.05_df[order(KEGG_enrich_0.05_df$p.adjust),]} #按照p.adjust排序
