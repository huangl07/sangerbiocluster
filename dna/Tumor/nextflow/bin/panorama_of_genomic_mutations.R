...
maf<-laml@data
pdf(paste0(opt$out, "/panorama_of_genomic_mutations.pdf"),height=10,width=10)
onco<-oncoplot(maf = laml,colors=vc_cols, gene_mar = 8, legend_height = 3)
dev.off()
png(paste0(opt$out, "/panorama_of_genomic_mutations.png"),height=1500,width=1500,unit="px",res=300)
onco<-oncoplot(maf = laml,colors=vc_cols, gene_mar = 8, legend_height = 3)
dev.off()
#pdf(paste0(opt$out, "/mutation_relation.pdf"))
#mr <- somaticInteractions(maf=laml,top=25,pvalue=c(0.05,1),nShiftSymbols=3)
#dev.off()
...
#ggsave(paste0(opt$out,"/plot_oncodriven.png"),p1)
#ggsave(paste0(opt$out,"/plot_oncodriven.pdf"),p1)
