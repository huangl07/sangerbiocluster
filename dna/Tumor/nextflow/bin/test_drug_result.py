...
def translate(vcf,database,output,name):
    ...
    with open (database,"r") as d:
        ...
        for lines in d:
            if ih == 0:
                ...
                gene_all = lines.strip().split("\t")[29:31]
                gene =":".join(gene_all)
                line = lines.strip().split("\t")
                out_line = []
                gene_name = line[0].split(" ")[0]
                protein = gene_name+":p."+line[0].split(" ")[1]
                out_line.append(gene_name)
                out_line.append(protein)
    ...
    with open(output,"w") as o:
        i = 0
        #o.write("Chrom\tPos\tRef\tAlt\tSymbol\tProtein_change\tDisease	Drug	Drug_Type	Drug_Directions	Evidence_Level	Significance\n")
                ...
                if line.startswith("#"):
                    ...
                    if gene_id in gene_dict:
                        ...
                        for z in range(len(gene_dict[gene_id])):
                            out_str +=  name+"\t"+ "\t".join(line.strip().split("\t")[0:2]) + "\t" + "\t".join(line.strip().split("\t")[3:5])+ "\t" + "\t".join(gene_dict[gene_id][z]) + "\n"
                        o.write(out_str)
def main():
    if len(sys.argv) != 5:
