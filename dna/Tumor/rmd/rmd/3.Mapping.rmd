...
tempdf <- map_stat_df[,c(1,2,3,5,7,8,11)]
colnames(tempdf)[3] <- "Properly Mapped(%)"
...
"Target Ratio(%)：目标区域对应reads的碱基总数占总碱基数的比例；",
"Region covered(%)：覆盖到的捕获区域。"
)
```
...
> **注：**横坐标表示测序深度，图中蓝色曲线，表示小于或等于该深度的位点占全基因组的百分比。
