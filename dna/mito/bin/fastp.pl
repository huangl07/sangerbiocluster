...
for (my $i=0;$i<@b1rawA;$i++) {
	$rawstat{AT}++ if(abs($b1rawA[$i]-$b1rawT[$i]) > $gc && $i > 10);
	$rawstat{GC}++ if(abs($b1rawG[$i]-$b1rawC[$i]) > $gc && $i > 10);
	print Out join("\t",$i,$b1rawA[$i],$b1rawT[$i],$b1rawG[$i],$b1rawC[$i],$b1rawN[$i]),"\n";
}
for (my $i=0;$i<@b2rawA;$i++) {
	$rawstat{AT}++ if(abs($b2rawA[$i]-$b2rawT[$i]) > $gc && $i > 10);
	$rawstat{GC}++ if(abs($b2rawG[$i]-$b2rawC[$i]) > $gc && $i > 10);
...
for (my $i=0;$i<@b1cleanA;$i++) {
	$cleanstat{AT}++ if(abs($b1cleanA[$i]-$b1cleanT[$i]) > $gc && $i > 10);
	$cleanstat{GC}++ if(abs($b1cleanG[$i]-$b1cleanC[$i]) > $gc && $i > 10);
	print Out join("\t",$i,$b1cleanA[$i],$b1cleanT[$i],$b1cleanG[$i],$b1cleanC[$i],$b1cleanN[$i]),"\n";
}
for (my $i=0;$i<@b2cleanA;$i++) {
	$cleanstat{AT}++ if(abs($b2cleanA[$i]-$b2cleanT[$i]) > $gc && $i > 10);
	$cleanstat{GC}++ if(abs($b2cleanG[$i]-$b2cleanC[$i]) > $gc && $i > 10);
	print Out join("\t",$i+scalar @b1cleanA,$b2cleanA[$i],$b2cleanT[$i],$b2cleanG[$i],$b2cleanC[$i],$b2cleanN[$i]),"\n";
}
close Out;
open Out,">$fOut.raw.qual\n";
for (my $i=0;$i<@b1rawA;$i++) {
	$rawstat{Q20}++ if($b1rawQ[$i] < 20 && $i > 10);
...
for (my $i=0;$i<@b1cleanA;$i++) {
	$rawstat{Q20}++ if($b1cleanQ[$i] < 20 && $i > 10);
	print Out join("\t",$i,$b1cleanQ[$i]),"\n";
}
for (my $i=0;$i<@b2cleanA;$i++) {
	$rawstat{Q20}++ if($b2cleanQ[$i] < 20 && $i > 10);
...
open Out,">$fOut.stat";
print Out join("\t","#sampleID","rawreads","rawdata","rawq20","rawq30","rawGC","ada","cleanreads","cleandata","cleanq20","cleanq30","cleanGC","dup","rawAT-seprate","rawGC-seprate","cleanAT-seprate","cleanGC-seprate","raw-lower","clean-lower"),"\n";
my @out;
push @out,$sample;
...
push @out,$rawstat{AT};
push @out,$rawstat{GC};
push @out,$cleanstat{AT};
push @out,$cleanstat{GC};
push @out,$rawstat{Q20};
push @out,$cleanstat{Q20};
print Out join("\t",@out),"\n";
close Out;
