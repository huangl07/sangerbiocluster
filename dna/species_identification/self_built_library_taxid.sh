# $1 需要创建的数据库名字
# $2 需要创建的数据库包含的物种taxid，一行一个


source ~/sg-users/yiwei.tang/tyw.rc
cat /mnt/lustre/users/sanger-dev/sg-users/yiwei.tang/test_data/nt/nucl.accession2taxid | csvtk -t grep -f taxid -P $2 | csvtk -t cut -f accession.version > taxa.giid.acc.txt
